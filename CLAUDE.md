...
- `cmd/mfi-gui/` — the Wails desktop app. `main.go` runs the window; `gui.go` is the bound `GUI` type (thin wrappers over `internal/app`); `frontend/dist/` is the vanilla UI; `build/` holds packaging config (icon, Info.plist). Presentation-only GUI features live in their own bound files and may shell out to host tools directly (kept out of the platform-agnostic core): `appmeta.go` (real app icon/name/version via `aapt`), `appdetails.go` (`dumpsys package` / `ideviceinstaller` details), `deviceroot.go` (root/jailbreak heuristics), `renderfile.go` (file tree, image/PDF/hex/highlighted rendering, open-externally).
